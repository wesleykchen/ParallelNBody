...
inline double inner_prod(double a, double b) {
  return a*b;
}
...
inline float inner_prod(float a, float b) {
  return a*b;
}
...
inline double normSq(double a) {
  return a*a;
}
/** Compute the squared L2 norm of a float */
inline float normSq(float a) {
  return a*a;
}
...
inline float norm_2(float a) {
  return norm(a);
}
/** Compute the L1 norm of a double */
inline double norm_1(double a) {
  return std::abs(a);
}
/** Compute the L1 norm of a float */
inline float norm_1(float a) {
  return std::abs(a);
}
/** Compute the L-infinity norm of a double */
inline double norm_inf(double a) {
  return std::abs(a);
}
/** Compute the L-infinity norm of a float */
inline float norm_inf(float a) {
  return std::abs(a);
}
...
template <typename T>
inline typename norm_type<complex<T> >::type normSq(const complex<T>& a) {
  return normSq(a.real()) + normSq(a.imag());
}
/** Compute the L2 norm of a complex */
template <typename T>
inline typename norm_type<complex<T> >::type norm(const complex<T>& a) {
  using std::sqrt;
  return sqrt(normSq(a));
}
template <typename T>
inline typename norm_type<complex<T> >::type norm_2(const complex<T>& a) {
  return norm(a);
}
/** Compute the L1 norm of a complex */
template <typename T>
inline typename norm_type<complex<T> >::type norm_1(const complex<T>& a) {
  return norm_1(a.real()) + norm_1(a.imag());
}
/** Compute the L-infinity norm of a complex */
template <typename T>
inline typename norm_type<complex<T> >::type norm_inf(const complex<T>& a) {
  using std::max;
...
template <std::size_t N, typename T>
inline typename norm_type<Vec<N,T> >::type normSq(const Vec<N,T>& a) {
...
template <std::size_t N, typename T>
inline typename norm_type<Vec<N,T> >::type norm(const Vec<N,T>& a) {
  using std::sqrt;
  return sqrt(normSq(a));
}
template <std::size_t N, typename T>
inline typename norm_type<Vec<N,T> >::type norm_2(const Vec<N,T>& a) {
  return norm(a);
}
/** Compute the L1 norm of a Vec */
template <std::size_t N, typename T>
inline typename norm_type<Vec<N,T> >::type norm_1(const Vec<N,T>& a) {
...
template <std::size_t N, typename T>
inline typename norm_type<Vec<N,T> >::type norm_inf(const Vec<N,T>& a) {
  using std::max;
